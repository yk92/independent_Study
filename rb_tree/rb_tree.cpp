...
{
    intptr_t left = reinterpret_cast<intptr_t>(leftp);
    intptr_t right = reinterpret_cast<intptr_t>(rightp);
...
void print_tree_helper(node n, int indent)
{
    ...
    if (n->color == BLACK)
        cout << reinterpret_cast<intptr_t>(n->key) << endl;
    else
        cout << "<" << reinterpret_cast<intptr_t>(n->key) << ">" << endl;
